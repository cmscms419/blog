...
- [Introduction](https://vulkan-tutorial.com/Loading_models#page_Introduction)
- [Library](https://vulkan-tutorial.com/Loading_models#page_Library)
- [Sample mesh](https://vulkan-tutorial.com/Loading_models#page_Sample-mesh)
- [Loading vertices and indices](https://vulkan-tutorial.com/Loading_models#page_Loading-vertices-and-indices)
- [Vertex deduplication](https://vulkan-tutorial.com/Loading_models#page_Vertex-deduplication)
...
An easy way to find such models is to look for 3D scans on [Sketchfab](https://sketchfab.com/). Many of the models on that site are available in OBJ format with a permissive license.
...
You should change the type of the indices from `uint16_t` to `uint32_t`, because there are going to be a lot more vertices than 65535. Remember to also change the [`vkCmdBindIndexBuffer`](https://www.khronos.org/registry/vulkan/specs/1.0/man/html/vkCmdBindIndexBuffer.html) parameter:
...
Faces은 임의의 수의 버텍스로 구성되며, 각 버텍스는 인덱스별 위치, 노멀 및/또는 텍스처 좌표를 나타냅니다.
...
`attrib` 컨테이너는 모든 위치, 법선 및 텍스처 좌표를 `attrib.vertices`, `attrib.normals` 및 `attrib.texcoords` 벡터에 보관합니다.

The `shapes` container contains all of the separate objects and their faces.

`shapes` 컨테이너에는 개별 개체와 그 면이 모두 들어 있습니다.

각 face은 vertices 배열로 구성되며, 각 vertex에는 위치, normal  및 texture coordinate attributes의 index가 포함되어 있습니다.
...
간단하게 하기 위해, 지금은 모든 버텍스가 유일하다고 가정하고, 따라서 간단한 자동 증가 인덱스를 사용하겠습니다.

인덱스 변수는 `vertex_index`, `normal_index` 및 `texcoord_index` 멤버를 포함하는 `tinyobj::index_t` 유형입니다.
...
Run your program now with optimization enabled (e.g. `Release` mode in Visual Studio and with the `-O3` compiler flag for GCC`). 

This is necessary, because otherwise loading the model will be very slow. You should see something like the following:

![](attachments/inverted_texture_coordinates.png)
...
많은 `vertices`이 여러 삼각형에 포함되어 있기 때문에 `vertices` 벡터에는 중복된 정점 데이터가 많이 포함되어 있습니다.

unique vertices만 보관하고 index buffer를 사용해 vertex가 나타날 때마다 재사용해야 합니다.

이를 구현하는 간단한 방법은 `map` 또는 `unordered_map`을 사용하여 unique vertices와 각 indices를 추적하는 것입니다:
...
Every time we read a vertex from the OBJ file, we check if we've already seen a vertex with the exact same position and texture coordinates before. If not, we add it to `vertices` and store its index in the `uniqueVertices` container.

OBJ 파일에서 버텍스를 읽을 때마다 이전에 정확히 동일한 위치와 텍스처 좌표를 가진 버텍스를 본 적이 있는지 확인합니다. 그렇지 않은 경우 버텍스에 추가하고 고유 버텍스 컨테이너에 인덱스를 저장합니다.

그런 다음 새 버텍스의 `indices`를 index에 추가합니다. 이전에 정확히 동일한 버텍스를 본 적이 있는 경우, `uniqueVertices`에서 해당 `indices`를 조회하고 해당 인덱스를 인덱스에 저장합니다.
...
`Vertex` 구조체와 같은 사용자 정의 유형을 해시 테이블의 키로 사용하려면 동등성 테스트와 해시 계산이라는 두 가지 함수를 구현해야 하므로 지금 당장 프로그램이 컴파일되지 않습니다. 전자는 `Vertex` 구조체에서 `==` 연산자를 오버라이드하여 쉽게 구현할 수 있습니다:
